...
<!DOCTYPE html>
<html lang="id">
<head><?php include 'head.php'; ?><title>Total Laptop — <?= APP_NAME ?></title></head>
<body>
<?php include 'navbar.php'; ?>
<div class="container-fluid px-4">
    <div class="page-header">
        <div>
            <h5><i class="bi bi-laptop me-2 text-primary"></i>Total Laptop</h5>
            <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="index.php">Dashboard</a></li><li class="breadcrumb-item active">Total Laptop</li></ol></nav>
        </div>
...
async function loadData() {
    const res = await fetch('api.php?action=get_laptops&filter=all');
    _laptopsData = await res.json();
    const container = document.getElementById('laptop-list');
    if (!_laptopsData.length) {
        container.innerHTML = '<div class="col-12"><div class="empty-state"><i class="bi bi-laptop"></i><p class="mt-2">Belum ada data laptop</p></div></div>';
        return;
    }
    container.innerHTML = _laptopsData.map(renderLaptopCard).join('');
}
function cetakPDF() { cetakPDFLaptop('Daftar Semua Laptop', _laptopsData); }
